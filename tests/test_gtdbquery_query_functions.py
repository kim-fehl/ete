...
def test_01tree_annotation(gtdb_dbfile):
    tree = PhyloTree('((c__Alicyclobacillia, c__Bacilli), s__Caballeronia udeis);',
                     sp_naming_function=lambda name: name)
    tree.annotate_gtdb_taxa(dbfile=gtdb_dbfile, taxid_attr='name')

    assert tree.props.get('sci_name') == 'd__Bacteria'

    firmicutes = tree['c__Bacilli'].up
    assert firmicutes.props.get('taxid') == 'p__Firmicutes'
    assert firmicutes.props.get('sci_name') == 'p__Firmicutes'
    assert firmicutes.props.get('rank') == 'phylum'
    assert firmicutes.props.get('named_lineage') == ['root', 'd__Bacteria', 'p__Firmicutes']

    caballeronia = tree['s__Caballeronia udeis']
    assert caballeronia.props.get('taxid') == 's__Caballeronia udeis'
    assert caballeronia.props.get('sci_name') == 's__Caballeronia udeis'
    assert caballeronia.props.get('rank') == 'species'
    assert caballeronia.props.get('named_lineage') == [
...
def test_02tree_annotation(gtdb_dbfile):
    ...
    tree = PhyloTree('((GB_GCA_011358815.1,RS_GCF_003948265.1),(GB_GCA_003344655.1),(GB_GCA_011056255.1));',
                     sp_naming_function=lambda name: name)
    tree.annotate_gtdb_taxa(dbfile=gtdb_dbfile, taxid_attr='species')

    assert tree.props.get('sci_name') == 'g__Korarchaeum'

    cryptofilum = tree['GB_GCA_011358815.1'].up
    assert cryptofilum.props.get('taxid') == 's__Korarchaeum cryptofilum'
    assert cryptofilum.props.get('sci_name') == 's__Korarchaeum cryptofilum'
    assert cryptofilum.props.get('rank') == 'species'
    assert cryptofilum.props.get('named_lineage') == [
    ...
    assert sp003344655.props.get('taxid') == 'GB_GCA_003344655.1'
    assert sp003344655.props.get('sci_name') == 's__Korarchaeum sp003344655'
    assert sp003344655.props.get('rank') == 'subspecies'
    assert sp003344655.props.get('named_lineage') == [
        'root', 'd__Archaea', 'p__Thermoproteota', 'c__Korarchaeia',
...
def test_03tree_annotation(gtdb_dbfile):
...
def test_04tree_annotation(gtdb_dbfile):
    ...
    tree.annotate_gtdb_taxa(dbfile=gtdb_dbfile, taxid_attr="gtdb_spcode")
...
def test_get_topology(gtdb_dbfile):
    ...
    assert sorted(tree.leaf_names()) == ['f__Korarchaeaceae', 'o__Peptococcales', 'p__Huberarchaeota']
...
def test_name_lineages(gtdb_dbfile):
    gtdb = GTDBTaxa(dbfile=gtdb_dbfile)

    out = gtdb.get_name_lineage(['RS_GCF_006228565.1'])

    assert out[0]['RS_GCF_006228565.1'] == [
    ...
    assert out[0]['o__Peptococcales'] == ['root', 'd__Bacteria', 'p__Firmicutes_B', 'c__Peptococcia', 'o__Peptococcales']


def test_get_rank(gtdb_dbfile):
    gtdb = GTDBTaxa(dbfile=gtdb_dbfile)
    ranks = gtdb.get_rank(['c__Thorarchaeia', 'RS_GCF_001477695.1'])
    # {'c__Thorarchaeia': 'class', 'RS_GCF_001477695.1': 'subspecies'}
    assert ranks == {'c__Thorarchaeia': 'class', 'RS_GCF_001477695.1': 'subspecies'}


def test_ignore_unclassified(gtdb_dbfile):
    # normal case
    gtdb = GTDBTaxa(dbfile=gtdb_dbfile)
    ...
    _, _, _= t.annotate_gtdb_taxa(dbfile=gtdb_dbfile, taxid_attr='name', ignore_unclassified=False)

    # case0
    t0 = t['GB_GCA_011364985.1']
    assert t0.props.get("taxid") == 'GB_GCA_011364985.1'
    assert t0.props.get("sci_name") == 's__SMTZ1-83 sp011364985'
    assert t0.props.get("rank") == 'subspecies'

    t0 = t0.up
    assert t0.props.get("taxid") == 's__SMTZ1-83 sp011364985'
    assert t0.props.get("sci_name") == 's__SMTZ1-83 sp011364985'
    assert t0.props.get("rank") == 'species'

    # case1
    ids = ['GB_GCA_001563325.1', 'GB_GCA_011364985.1'] # soley species
    t1 = t.common_ancestor(ids)
    assert t1.props.get("taxid") == 'g__SMTZ1-83'
    assert t1.props.get("sci_name") == 'g__SMTZ1-83'
    assert t1.props.get("rank") == 'genus'
    assert t1.props.get("named_lineage") == ['root', 'd__Archaea', 'p__Asgardarchaeota', 'c__Thorarchaeia', 'o__Thorarchaeales', 'f__Thorarchaeaceae', 'g__SMTZ1-83']

    # case2
    ids = ['GB_GCA_003662765.1', 'GB_GCA_003662805.1'] # normal hierarchy
    t2 = t.common_ancestor(ids)
    assert t2.props.get("taxid") == 's__B65-G9 sp003662765'
    assert t2.props.get("sci_name") == 's__B65-G9 sp003662765'
    assert t2.props.get("rank") == 'species'
    assert t2.props.get("named_lineage") == ['root', 'd__Archaea', 'p__Asgardarchaeota', 'c__Thorarchaeia', 'o__Thorarchaeales', 'f__Thorarchaeaceae', 'g__B65-G9', 's__B65-G9 sp003662765']

    # case3
    ids = ['GB_GCA_002825535.1', 'GB_GCA_004524435.1']
    t3 = t.common_ancestor(ids)
    assert t3.props.get("taxid") == 'f__Thorarchaeaceae'
    assert t3.props.get("sci_name") == 'f__Thorarchaeaceae'
    assert t3.props.get("rank") == 'family'
    ...
    # check nomarl leaf
    assert t['GB_GCA_011364985.1'].props.get("taxid") == 'GB_GCA_011364985.1'
    assert t['GB_GCA_011364985.1'].props.get("sci_name") == 's__SMTZ1-83 sp011364985'

    # check unclassified leaf
    assert t['sample1'].props.get("taxid") == 'sample1'
    assert t['sample1'].props.get("sci_name") == ''
    assert t['sample1'].props.get("rank") == 'Unknown'
    assert t['sample1'].props.get("named_lineage") == []

    # check ancestor (should be empty)
    ids = ['sample1', 'GB_GCA_011364985.1']
    t1 = t.common_ancestor(ids)
    assert t1.props.get("taxid") == None
    assert t1.props.get("sci_name") == 'None'
    assert t1.props.get("rank") == 'Unknown'
    assert t1.props.get("named_lineage") == ['']

    assert t.props.get("taxid") == None
    assert t.props.get("sci_name") == 'None'
    assert t.props.get("rank") == 'Unknown'
    assert t.props.get("named_lineage") == ['']

    # ignore unclassified is True
    c__Thorarchaeia = '(((((((GB_GCA_002825535.1),(GB_GCA_003345545.1),(GB_GCA_002825465.1),(GB_GCA_004524565.1),(GB_GCA_004524595.1)),((GB_GCA_011364985.1),(GB_GCA_011365025.1),(GB_GCA_001563325.1)),((GB_GCA_004524445.1)),((GB_GCA_008080745.1)),((GB_GCA_003345595.1),(GB_GCA_003345555.1)),((GB_GCA_013388835.1)),((GB_GCA_003662765.1,GB_GCA_003662805.1)),((GB_GCA_011364905.1),(GB_GCA_001563335.1),(GB_GCA_001940705.1),(GB_GCA_004376265.1),(GB_GCA_002825515.1)),((GB_GCA_013138615.1)),((GB_GCA_004524435.1))))));'
    t = PhyloTree(c__Thorarchaeia)
    t['GB_GCA_001563325.1'].name = 'sample1'
    _, _, _= t.annotate_gtdb_taxa(dbfile=gtdb_dbfile, taxid_attr='name', ignore_unclassified=True)
    ...
    assert t.props.get("taxid") == 'f__Thorarchaeaceae'
    assert t.props.get("sci_name") == 'f__Thorarchaeaceae'
    assert t.props.get("rank") == 'family'
