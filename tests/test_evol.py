...
class TestEvolEvolTree:
    ...
    def test_paml_parser(self):
        ...
        for ali in alignments:
            ...
            assert (t & 'seq1').props.get('nt_sequence') == 'ATGATG'
            assert (t & 'seq2').props.get('nt_sequence') == 'CTGATG'
            assert (t & 'seq3').props.get('nt_sequence') == 'ATGTTT'
    ...
    def test_load_model(self):
        ...
        assert sorted(tree._models.keys()) == sorted(['fb', 'M1', 'M2', 'M7', 'M8'])
        assert len (tree.get_evol_model('M2').branches) == 194
        assert tree.get_evol_model('fb').lnL == -3265.316569
        assert 'proportions' in str(tree.get_evol_model('M2'))
        assert 'p2=' in str(tree.get_evol_model('M2'))
        assert 'proportions' not in str(tree.get_evol_model('fb'))
        assert ' #193' in str(tree.get_evol_model('fb'))
    ...
    def test_get_most_likely(self):
        ...
        tree.link_to_evol_model (WRKDIR + 'paml/M1/M1.out', 'M1')
        tree.link_to_evol_model (WRKDIR + 'paml/M2/M2.out', 'M2')
        assert round(tree.get_most_likely ('M2' == 'M1',16),
    ...
    def test_deep_copy(self):
        ...
        assert str(tree.get_evol_model('fb.a')) == str(tree.get_evol_model('fb')
                     )
    def test_call_histface(self):
        ...
        try:
            ...
            assert str(type(M2a.properties['histface'])) == "<class 'ete3.treeview.faces.SequencePlotFace'>"
    ...
    def test_pickling(self):
        ...
        assert tree_output == tree2_output
